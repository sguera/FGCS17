...
int main (int argc, char** argv)
{
	...
	if (((WIDTH%4)!=0))
	{
		printf("Non-native, aligned SIMD type mode requires that WIDTH is divisible by 4 [WIDTH = %ld].\n", WIDTH);
		return -1;
	}
	U_0_0=((float * )malloc((((WIDTH*HEIGHT)*sizeof (float))+15)));
	U_0_0_ref=((float * )malloc((((WIDTH*HEIGHT)*sizeof (float))+15)));
	if (((WIDTH%4)!=0))
	{
		printf("Non-native, aligned SIMD type mode requires that WIDTH is divisible by 4 [WIDTH = %ld].\n", WIDTH);
		return -1;
	}
	U_0_1=((float * )malloc((((WIDTH*HEIGHT)*sizeof (float))+15)));
	U_0_1_ref=((float * )malloc((((WIDTH*HEIGHT)*sizeof (float))+15)));
	// <--
...
#pragma omp parallel
	{
		// initialize_grids -->
		initialize_blur_parm(((float * )((((uintptr_t)U_0_0)+15)&( ~ ((uintptr_t)15)))), ((float * )((((uintptr_t)U_0_1)+15)&( ~ ((uintptr_t)15)))), 3.0, 0.05555555555555555, exp(1.0), exp(-0.05555555555555555), exp(-0.1111111111111111), exp(-0.2222222222222222), exp(-0.2777777777777778), exp(-0.4444444444444444), (1.0/(exp(1.0)+((((exp(-0.05555555555555555)+exp(-0.1111111111111111))+(exp(-0.2222222222222222)+exp(-0.4444444444444444)))*4.0)+(exp(-0.2777777777777778)*8.0)))), WIDTH, HEIGHT, cb_x, cb_y, chunk);
	...
	{
		// write_grids -->
		write_data_f("U_0_0.0.data", 2, ((float * )((((uintptr_t)U_0_0)+15)&( ~ ((uintptr_t)15)))), WIDTH, HEIGHT);
		write_data_f("U_0_1.0.data", 2, ((float * )((((uintptr_t)U_0_1)+15)&( ~ ((uintptr_t)15)))), WIDTH, HEIGHT);
		// <--
...
#pragma omp parallel
	{
		// compute_stencil -->
		blur_parm(( & U_0_1_out), ((float * )((((uintptr_t)U_0_0)+15)&( ~ ((uintptr_t)15)))), ((float * )((((uintptr_t)U_0_1)+15)&( ~ ((uintptr_t)15)))), 3.0, 0.05555555555555555, exp(1.0), exp(-0.05555555555555555), exp(-0.1111111111111111), exp(-0.2222222222222222), exp(-0.2777777777777778), exp(-0.4444444444444444), (1.0/(exp(1.0)+((((exp(-0.05555555555555555)+exp(-0.1111111111111111))+(exp(-0.2222222222222222)+exp(-0.4444444444444444)))*4.0)+(exp(-0.2777777777777778)*8.0)))), WIDTH, HEIGHT, cb_x, cb_y, chunk, _unroll_p3);
		// <--
